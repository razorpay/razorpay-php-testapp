---
title: Approve a Collect Request
description: Approve a collect request using the Razorpay TPAP Pro API.
---
...
```curl: Curl
curl -X POST 'api.rzp..com/v1/payments/:upi_transaction_id/approve' \
...
: `string` The unique identifier of the transaction across all entities in UPI created by the originator. In payments, the lifecycle starts from CL, so it is mandatory for the originator to create it. All further actions regarding this payment will be done using this ID.
