...
String paymentId = "pay_JGmCgTEa9OTQcX";

List payments = razorpay.payments.fetchAllTransfers(paymentId)

```php: PHP
$api = new Api($key_id, $secret);

$api->payment->fetch('payment')->transfers();
...
body, err := client.Payment.Transfers("", nil, nil)
...
List token = client.Payment.Fetch(paymentId).Transfers();
```
