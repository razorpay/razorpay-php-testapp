...
```javascript: Node.js
var instance = new Razorpay({ key_id: 'YOUR_KEY_ID', key_secret: 'YOUR_SECRET', headers : { 'Content-Type': 'application/json' }})
...
List paymentlink = razorpay.paymentLink.fetchAll(params);
...
: `string` Unique identifier of the payment associated with the Payment Link.

`reference_id` _optional_
: `string` The unique reference number entered while creating the Payment Link.
